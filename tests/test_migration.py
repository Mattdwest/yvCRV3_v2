...
from brownie import Strategy3Poolv2, Strategy3Pool2
...
def test_migration(pm, chain):
    ...
    newstrategy = guardian.deploy(Strategy3Pool2, yCRV3, dai, threePool, crv3, crv, unirouter, weth, proxy, gauge)
    newstrategy.setStrategist(strategist)

    yCRV3.migrateStrategy(strategy, newstrategy, {"from": gov})

    assert crv3.balanceOf(strategy) == 0
    assert crv3.balanceOf(newstrategy) > 0
